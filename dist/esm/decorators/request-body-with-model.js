...
export function requestBodyWithModel(model) {
    ...
    if (!isClass(modelClass))
        throw new Errorf('The first argument of @requestBodyWithModel must be a model class, ' +
    ...
    const modelSchema = getDataSchemaByModelClass(modelClass, ProjectionScope.INPUT);
    if (isArray)
        return requestBody({ type: DataType.ARRAY, items: modelSchema });
