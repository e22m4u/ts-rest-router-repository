 ...
 * @param modelInput
 */
export function extractModelClassFromDecoratorInput(decoratorName, modelInput) {
    ...
    if (Array.isArray(modelInput)) {
        ...
        if (modelInput.length !== 1) {
            throw new Errorf('If an array (or a factory returning an array) is passed to @%s, ' +
                'it must contain exactly one model class, but %v items given.', decoratorName, modelInput.length);
        }
    ...
    if (!isClass(modelClass))
        throw new Errorf('The first argument of @%s must be a model class, ' +
            'an array containing a single model class, ' +
            'or a factory function of these values.', decoratorName);
